...
export default function InstallButton({
  ...
  const gradientColors = platform === 'ios' 
  ...
  const getSubText = () => {
    if (platform === 'ios') {
      return deviceType === 'desktop' ? 'Scan QR Code or visit on device' : 'Tap to install';
    }
  ...
  return (
    <motion.a
      ...
      whileHover={isDownloading ? {} : { scale: 1.03, y: -3 }}
    ...
    >
      ...
      <div className="flex flex-col items-start min-w-[140px] relative z-10">
        <span className="text-white font-bold text-base tracking-tight mb-1">
          {isDownloading ? 'Installing...' : getButtonText()}
        </span>
      ...
      <motion.div 
        ...
        animate={isHovered && !isDownloading ? { opacity: 1, x: 0 } : { opacity: 0, x: -10 }}
