...
import { useBuildAnalytics, useBuildShareLinks, useBuildFeedback, useUpdateBuild } from '@/hooks/queries';
import type { Build, ShareLink, BuildAnalyticsSummary, LinkType, BuildFeedback } from '@/types';
...
export default function EnhancedBuildManagement({ build: initialBuild }: BuildManagementProps) {
  const router = useRouter();
  const toast = useToast();
  const queryClient = useQueryClient();
  const [build, setBuild] = useState(initialBuild);
  const [activeTab, setActiveTab] = useState<'overview' | 'analytics' | 'share' | 'branding' | 'feedback' | 'settings'>('overview');
  ...
  const { data: analytics, isLoading: isLoadingAnalytics } = useBuildAnalytics(activeTab === 'analytics' ? build.id : '');
  const { data: shareLinksData = initialBuild.share_links || [], refetch: refetchShareLinks } = useBuildShareLinks(activeTab === 'share' ? build.id : '');
  const { data: feedbackList = [] } = useBuildFeedback(activeTab === 'feedback' ? build.id : '');
  
  // Maintain local state for share links so we can update it optimistically
  const [shareLinks, setShareLinks] = useState<ShareLink[]>(initialBuild.share_links || []);
  ...
  const createShareLink = async () => {
    ...
    try {
      const data = await fetchFromLaravel<{ success: boolean; data: ShareLink, error?: string }>(`/api/builds/${build.id}/share-links`, {
        method: 'POST',
      ...
      if (data.success) {
        setShareLinks([{ ...data.data, last_used_at: null, unique_devices: 0 }, ...shareLinks]);
        setNewLinkLabel('');
  ...
  const platformIcon = build.platform === 'ios' ? (
    <Image src="/images/logo/apple-logo-svgrepo-com.svg" alt="iOS" width={24} height={24} className="w-6 h-6 text-current" />
  ) : (
    <Image src="/images/logo/android-logo-svgrepo-com.svg" alt="Android" width={24} height={24} className="w-6 h-6 text-current" />
  );

  const getLinkTypeColor = (type: LinkType) => {
    switch (type) {
      case 'qa': return 'text-blue-400 bg-blue-500/10 border-blue-500/20';
      case 'stakeholder': return 'text-purple-400 bg-purple-500/10 border-purple-500/20';
      case 'beta_tester': return 'text-green-400 bg-green-500/10 border-green-500/20';
      case 'reviewer': return 'text-orange-400 bg-orange-500/10 border-orange-500/20';
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-start space-x-4">
            <div className={`w-16 h-16 rounded-2xl flex items-center justify-center overflow-hidden relative ${
              logoUrl
                ? 'bg-transparent'
                : (build.platform === 'ios' ? 'bg-gradient-to-br from-gray-600 to-gray-800 text-white' : 'bg-gradient-to-br from-gray-600 to-gray-800 text-white')
            }`}>
              {logoUrl ? (
                 <Image 
                   src={logoUrl} 
            ...
            <div>
              ...
              <div className="flex items-center space-x-3 text-sm text-white/60">
                ...
                {build.build_number && build.build_number !== '0' && <><span>•</span><span>Build {build.build_number}</span></>}
                <span>•</span>
          ...
          <div className="mt-4 p-3 rounded-lg bg-orange-500/10 border border-orange-500/20 flex items-start gap-2">
            ...
            <div className="text-sm">
              ...
              <div className="text-orange-400/80">
                ...
                {build.expiry_download_limit && ` • ${build.download_count}/${build.expiry_download_limit} downloads`}
                {build.expiry_device_limit && ` • ${build.unique_devices_count}/${build.expiry_device_limit} devices`}
              </div>
            </div>
          </div>
        )}
      ...
      {activeTab === 'overview' && (
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="card">
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                ...
                >
                  {isEnabled ? (
                    <><ToggleRight className="w-5 h-5 mr-2 text-green-400" /> Disable Build</>
                  ) : (
                    <><ToggleLeft className="w-5 h-5 mr-2 text-white/40" /> Enable Build</>
                  )}
                </button>
            ...
            <div className="card">
              ...
              <div className="space-y-3">
                <div className="flex justify-between py-2 border-b border-white/5">
                  ...
                  <span className="text-white font-medium capitalize">{build.platform}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-white/5">
                ...
                {build.build_number && (
                  <div className="flex justify-between py-2 border-b border-white/5">
                    ...
                    <span className="text-white font-medium">{build.build_number}</span>
                  </div>
                )}
                <div className="flex justify-between py-2 border-b border-white/5">
                  <span className="text-white/60">File Size</span>
                  <span className="text-white font-medium">{formatFileSize(build.file_size)}</span>
                </div>
                {build.bundle_id && (
                  <div className="flex justify-between py-2 border-b border-white/5">
                    <span className="text-white/60">Bundle ID</span>
                    <span className="text-white font-medium font-mono text-sm">{build.bundle_id}</span>
                  </div>
                )}
                <div className="flex justify-between py-2 border-b border-white/5">
                  <span className="text-white/60">Created</span>
                  <span className="text-white font-medium">{formatRelativeTime(build.created_at)}</span>
                </div>
                <div className="flex justify-between py-2">
                  <span className="text-white/60">Last Updated</span>
                  <span className="text-white font-medium">{formatRelativeTime(build.updated_at)}</span>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="card">
              ...
              <div className="space-y-4">
                <div className="bg-white/5 rounded-lg p-4">
                  ...
                  <div className="text-3xl font-bold text-white">{build.download_count}</div>
                </div>
                ...
                <div className="bg-white/5 rounded-lg p-4">
                  ...
                  <div className="text-3xl font-bold text-white">{build.unique_devices_count}</div>
                </div>
                ...
                <div className="bg-white/5 rounded-lg p-4">
                  ...
                  <div className="text-3xl font-bold text-white">
                    {build.download_count > 0 
                      ? Math.round((build.install_success_count / build.download_count) * 100)
                      : 0}%
                  </div>
                </div>
              </div>
            ...
            {/* Share Link */}
            <div className="card">
              <h2 className="text-lg font-semibold text-white mb-4">Share Link</h2>
              <div className="bg-white/5 rounded-lg p-3 mb-3">
                <div className="text-xs text-white/40 mb-1">Install URL</div>
                <div className="text-sm text-white font-mono break-all">{installUrl}</div>
              </div>
              <button
            ...
            <>
              ...
              <div className="grid md:grid-cols-4 gap-6">
                <div className="card">
                  ...
                  <div className="text-3xl font-bold text-white">{analytics.total_downloads}</div>
                </div>
                ...
                <div className="card">
                  ...
                  <div className="text-3xl font-bold text-white">{analytics.unique_devices}</div>
                </div>
                ...
                <div className="card">
                  ...
                  <div className="text-3xl font-bold text-white">
                    {Math.round(analytics.install_success_rate)}%
                  </div>
                </div>
                ...
                <div className="card">
                  ...
                  <div className="text-3xl font-bold text-white">
                    {analytics.geographic_distribution.length}
                  </div>
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="card">
                  <h3 className="text-lg font-semibold text-white mb-4">Device Type Breakdown</h3>
                  <div className="space-y-3">
                    {Object.entries(analytics.device_breakdown)
                          ...
                          <div className="flex items-center gap-3">
                            <div className="w-32 h-2 bg-white/10 rounded-full overflow-hidden">
                              <div 
                                ...
                                style={{ width: `${(count / analytics.total_downloads) * 100}%` }}
                              />
                            </div>
                      ...
                      <p className="text-white/40 text-sm italic">No device data available</p>
                    )}
                  </div>
                </div>
                ...
                <div className="card">
                  ...
                  <div className="space-y-3">
                    {Object.entries(analytics.platform_breakdown)
                      ...
                      <p className="text-white/40 text-sm italic">No platform data available</p>
                    )}
                  </div>
                </div>
              </div>
                  ...
                  <div className="space-y-2">
                    {analytics.top_devices.map((device, index) => (
                      <div key={index} className="flex items-center justify-between py-2 border-b border-white/5 last:border-0">
                        ...
                        <span className="text-white font-medium">{device.count} installs</span>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="space-y-2">
                    {analytics.geographic_distribution.map((geo, index) => (
                      <div key={index} className="flex items-center justify-between py-2 border-b border-white/5 last:border-0">
                        ...
                        <span className="text-white font-medium">{geo.count} installs</span>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      {activeTab === 'share' && (
        <div className="space-y-6">
          ...
          <div className="card relative z-20">
            <h3 className="text-lg font-semibold text-white mb-4">Create Role-Based Share Link</h3>
            <div className="grid md:grid-cols-2 gap-4 mb-4">
              <div>
                <label className="block text-sm text-white/60 mb-2">Link Type</label>
                <Select
                  value={newLinkType}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setNewLinkMaxUses(e.target.value ? parseInt(e.target.value) : undefined)}
          ...
          <div className="card">
            ...
            {shareLinks.length > 0 ? (
              <div className="space-y-3">
                {shareLinks.map((link) => (
                  <div key={link.id} className="p-4 rounded-lg bg-white/5 border border-white/10 hover:bg-white/[0.07] transition-colors">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium border ${getLinkTypeColor(link.link_type)}`}>
                      ...
                      <div className="text-right text-sm">
                        ...
                        <div className="text-xs text-white/40 mt-1">
                          ...
                          {link.last_used_at && ` • Last use ${formatRelativeTime(link.last_used_at)}`}
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => copyLink(`${appUrl}/install?i=${link.short_id || link.token}`, link.id)}
      ...
      {activeTab === 'branding' && (
        <BuildBranding build={build} onUpdate={(updatedBuild) => setBuild(updatedBuild)} />
      )}

      {activeTab === 'settings' && (
        <BuildSettings build={build} onUpdate={(updatedBuild) => setBuild(updatedBuild)} />
      )}
          ...
          {feedbackList.length > 0 ? (
            <div className="grid gap-4">
              {feedbackList.map((item) => {
                ...
                return (
                  <div key={item.id} className="group relative overflow-hidden rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-6 hover:bg-white/10 transition-all duration-300">
                     <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-4">
                      <div className="space-y-1">
                        <div className="flex flex-wrap items-center gap-2">
                          <h3 className="text-lg font-bold text-white">{item.title}</h3>
                          <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border capitalize ${
                            item.feedback_type === 'bug' ? 'bg-rose-500/10 text-rose-400 border-rose-500/20' :
                            item.feedback_type === 'suggestion' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' :
                            'bg-gray-500/10 text-gray-400 border-gray-500/20'
                          }`}>
                            {getFeedbackIcon(item.feedback_type)}
                            {item.feedback_type}
                          </span>
                          <span className={`px-2.5 py-1 rounded-full text-xs font-semibold border capitalize ${getSeverityColor(item.severity)}`}>
                            {item.severity} Priority
                          </span>
                          
                          {item.rating && item.rating > 0 && (
                            <div className="flex items-center gap-0.5 px-2.5 py-1 rounded-full bg-yellow-500/10 border border-yellow-500/20" title={`Rating: ${item.rating}/5`}>
                              {[...Array(5)].map((_, i) => (
                                <Star 
                                  key={i} 
                              ...
                              <span className="ml-1 text-xs font-bold text-yellow-500">{item.rating}</span>
                            </div>
                          )}
                        </div>
                        <div className="flex items-center gap-2 text-sm text-white/40">
                          <span>Reported by <span className="text-white/70 font-medium">{item.reporter_name || item.reporter_email || 'Anonymous'}</span></span>
                          <span>•</span>
                      ...
                      {item.status && (
                        <div className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border ${
                          item.status === 'new' ? 'bg-primary-500/10 text-primary-400 border-primary-500/20' :
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      {item.device_info && (
                        <div className="flex items-center gap-2 text-xs font-mono text-white/50 bg-white/5 px-3 py-1.5 rounded-lg border border-white/5">
                          ...
                          <span className="truncate max-w-xs">{item.device_info}</span>
                        </div>
                      )}
            ...
            <div className="group relative rounded-2xl border border-white/10 bg-white/5 p-12 text-center overflow-hidden">
               ...
               <div className="relative z-10">
                ...
                <p className="text-gray-400 max-w-sm mx-auto">
                  Feedback from your testers will appear here. Share your build to get started!
                </p>
              </div>
            </div>
          )}
