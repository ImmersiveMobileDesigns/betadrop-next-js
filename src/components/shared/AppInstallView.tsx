...
const SectionHeader = ({ icon: Icon, title, subtitle }: { icon: React.ElementType; title: string, subtitle?: string }) => (
  <div className="flex items-center gap-4 mb-6">
    ...
    <div className="flex flex-col">
      ...
      {subtitle && <span className="text-[10px] sm:text-xs font-medium opacity-50 uppercase tracking-wider">{subtitle}</span>}
    </div>
  </div>
);

// Stat Card Component for Overview
const StatCard = ({ label, value, icon: Icon, delay = 0 }: { label: string; value: string; icon: any; delay?: number }) => (
  <motion.div 
  ...
  >
    ...
    <span className="font-bold text-base sm:text-lg truncate" title={value}>{value}</span>
  </motion.div>
);

// Data Row Component
const DataRow = ({ label, value, isLink = false, copyable = false }: { label: string; value: string; isLink?: boolean; copyable?: boolean }) => {
  ...
  return (
    <div className="flex items-center justify-between py-4 border-b border-white/5 last:border-0 group hover:bg-white/[0.02] px-2 -mx-2 rounded-lg transition-colors">
      ...
      <div className="flex items-center gap-2">
        {isLink ? (
          <a href={value} target="_blank" rel="noopener noreferrer" className="hover:opacity-80 text-xs sm:text-sm font-semibold flex items-center gap-1 transition-opacity" style={{ color: 'var(--primary-color)' }}>
            {value} <ExternalLink className="w-3 h-3" />
          </a>
        ) : (
          <span className="text-xs sm:text-sm font-semibold text-right">{value}</span>
        )}
        {copyable && (
          <button 
            onClick={handleCopy}
          ...
          >
            {copied ? <Check className="w-3.5 h-3.5 text-green-400" /> : <Copy className="w-3.5 h-3.5 opacity-40 hover:opacity-100" />}
          </button>
        )}
      </div>
    </div>
...
const ExpandableRow = ({ label, value, count, children }: { label: string; value?: string; count?: number; children?: React.ReactNode }) => {
  ...
  return (
    <div className="border-b border-white/5 last:border-0">
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {value && <span className="text-xs sm:text-sm font-medium">{value}</span>}
          {count !== undefined && (
            <span className="px-2 py-0.5 text-xs rounded-full font-medium" style={{ backgroundColor: 'var(--primary-color)33', color: 'var(--primary-color)' }}>{count}</span>
          )}
          {children && (
...
const FAQItem = ({ question }: { question: string }) => {
  ...
  const answers: Record<string, string> = {
    'Why is the app asking for storage permissions?': 'The app may request storage permissions to save data locally on your device. This is standard behavior for many apps.',
    "The app won't open after installation. What should I do?": 'Go to Settings → General → VPN & Device Management, find the developer certificate, and tap "Trust" to allow the app to run.',
    'Is the app available on iOS as well?': 'This depends on the app publisher. Check with the developer for cross-platform availability.',
    'Why do I see a security warning during installation?': 'This is normal for apps distributed outside the App Store. You need to trust the developer certificate in your device settings.',
    "How do I update the app?": "When a new version is available, you'll receive a new download link. Install it over the existing version to update.",
  };
        ...
        {isOpen && (
          ...
          >
            <p className="pb-3 text-xs sm:text-sm opacity-50 leading-relaxed">
              {answers[question] || 'Please contact the app developer for more information.'}
            </p>
          </motion.div>
        )}
      </AnimatePresence>
...
export default function AppInstallView({
  ...
  useEffect(() => {
    ...
    setFormattedDate(createdDate.toLocaleString('en-US', { 
      month: 'short', 
      day: 'numeric', 
      year: 'numeric',
      hour: '2-digit',
    ...
    if (data.expiresAt) {
      const calculateTimeRemaining = () => {
        ...
        setTimeRemaining({
          ...
          hours: Math.max(0, Math.floor((remaining % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)))
        });
      };
  ...
  const bgColor = data.customBackgroundColor || (data.customThemeMode === 'light' ? '#f8fafc' : '#0f172a');
  const textColor = data.customThemeMode === 'light' ? '#1e293b' : '#f8fafc';
  const subTextColor = data.customThemeMode === 'light' ? '#64748b' : '#94a3b8';

  const themeClass = data.customThemeMode === 'light' ? 'light-theme' : 'dark-theme';

  return (
    <div className={`min-h-screen transition-colors duration-300 ${themeClass}`} style={{ 
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className={`absolute -top-1/2 -right-1/2 w-full h-full rounded-full blur-2xl opacity-10 ${
          platform === 'ios' ? 'bg-gradient-to-br from-blue-600/40 to-purple-600/20' : 'bg-gradient-to-br from-green-600/40 to-emerald-600/20'
        }`} />
        <div 
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 py-6 lg:py-10">
        ...
        {!data.hidePlatformBranding && (
          ...
          >
            ...
            >
              <span className="text-[10px] sm:text-xs font-bold opacity-60 uppercase tracking-widest group-hover:opacity-90 transition-opacity leading-none pt-0.5">Powered by</span>
        ...
        {data.customMessage && (
          <motion.div
            ...
            style={{ backgroundColor: 'var(--primary-color)20', borderColor: 'var(--primary-color)40' }}
          >
            <p className="text-xs sm:text-sm font-semibold" style={{ color: 'var(--primary-color)' }}>
              {data.customMessage}
            </p>
          </motion.div>
        )}
        ...
        >
          ...
          <div className="relative rounded-2xl p-5 lg:p-7 overflow-hidden" style={{ backgroundColor: 'var(--card-bg)' }}>
              ...
              <div className="flex items-center gap-5">
                ...
                <div className="space-y-3">
                  ...
                  >
                    ...
                    <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wide shadow-lg backdrop-blur-sm transition-transform hover:scale-105 ${
                      platform === 'ios' 
                    ...
                    >
                      ...
                      v{data.version}
                      {data.buildNumber && <span className="opacity-70">({data.buildNumber})</span>}
                    </span>
                ...
                <div className="flex items-center gap-4 p-4 rounded-xl bg-gradient-to-r from-blue-500/10 via-blue-500/5 to-transparent border border-blue-500/20 backdrop-blur-sm">
                  ...
                  <div className="flex-1">
                    <p className="text-xs sm:text-sm font-medium opacity-80">
                      Open this page on your <strong className="text-blue-400 font-semibold">iPhone or iPad</strong> to install directly via OTA
                    </p>
                    <p className="text-xs opacity-50 mt-0.5">Scan the QR code or share the link</p>
                  </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            <motion.div 
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.1 }}
              className="group rounded-2xl border p-4 sm:p-6 backdrop-blur-md shadow-sm transition-all duration-300"
              style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--card-border)' }}
            >
              <SectionHeader icon={Activity} title="App Overview" subtitle="Quick Stats" />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <StatCard icon={Box} label="App Name" value={data.appName} delay={0.1} />
                <StatCard icon={Hash} label="Version" value={`v${data.version} (${data.buildNumber || '1'})`} delay={0.2} />
                <StatCard icon={FileBox} label="Size" value={formatFileSize(data.fileSize)} delay={0.3} />
                <StatCard icon={Calendar} label="Uploaded" value={formattedDate} delay={0.4} />
              </div>
            </motion.div>
            ...
            {data.notes && (
              <motion.div 
                ...
                style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--card-border)' }}
              >
                <SectionHeader icon={FileText} title="Release Notes" subtitle="What's New" />
   ...
   <motion.div 
              ...
              style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--card-border)' }}
            >
              <SectionHeader icon={Settings} title="Technical Details" subtitle="Configuration" />
              <div className="space-y-0">
                <DataRow label="Package" value={data.packageName} copyable />
                <DataRow label="Minimum OS Version" value={data.minOSVersion || (platform === 'ios' ? '14.0' : '7.0')} />
                <ExpandableRow label="Required Device Capabilities" count={platform === 'ios' ? 2 : 1}>
                ...
                <ExpandableRow 
                  ...
                  count={data.provisionedDevices?.length || (platform === 'ios' ? 0 : undefined)} 
                  value={platform === 'android' ? 'All' : undefined}
                >
                  {platform === 'ios' && data.provisionedDevices && data.provisionedDevices.length > 0 ? (
                    <div className="mt-2 space-y-1 max-h-48 overflow-y-auto">
                      {data.provisionedDevices.map((device, index) => (
                        <div key={index} className="text-xs font-mono opacity-80 bg-white/5 p-1.5 rounded select-all hover:bg-white/10 transition-colors">
                          {device}
                        </div>
                      ))}
                    </div>
                  ) : platform === 'ios' ? (
                     <p className="opacity-40 text-xs text-center border-t border-white/5 pt-2 mt-2">No provisioned devices found in this build.</p>
                  ) : null}
                </ExpandableRow>
                <DataRow label="Supported Architectures" value={data.supportedArchitectures?.join(', ') || 'arm64'} />
                <DataRow label="Debuggable" value={data.isDebuggable ? 'Yes' : 'No'} />
              </div>
            </motion.div>
            ...
            <motion.div 
              ...
              style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--card-border)' }}
            >
              <SectionHeader icon={Shield} title="Features & Privacy" subtitle="Permissions" />
              <div className="space-y-0">

                <ExpandableRow label="Permissions" count={data.permissions?.length || 0}>
                   {data.permissions && data.permissions.length > 0 ? (
                    <ul className="list-disc list-inside space-y-1">
                     ...
                     <span className="text-xs opacity-50 pl-1">No special permissions detected</span>
                   )}
                </ExpandableRow>
                ...
                <ExpandableRow label="Device Families" count={data.deviceFamilies?.length || 2}>
                  <ul className="list-disc list-inside space-y-1">
                    {(data.deviceFamilies || (platform === 'ios' ? ['iPhone', 'iPad'] : ['Phone', 'Tablet'])).map((family) => (
            ...
            <motion.div 
              ...
              style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--card-border)' }}
            >
              <SectionHeader icon={MessageSquare} title="Feedback" subtitle="Rate Experience" />
              ...
              {feedbackSubmitted ? (
                 <div className="flex flex-col items-center justify-center py-8 text-center">
                   ...
                   <p className="text-sm opacity-60">Your feedback has been submitted successfully.</p>
                 </div>
              ) : (
                <div className="space-y-4">
                  <div className="flex flex-col items-center gap-2 mb-4">
                    <span className="text-sm font-medium opacity-70">Rate this build</span>
                    <div className="flex items-center gap-2">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <motion.button
                          key={star}
                  ...
                  <div className="space-y-3">
                    ...
                    <motion.input
                      ...
                      animate={highlightEmail ? { x: [0, -4, 4, -4, 4, 0] } : {}}
                      transition={{ duration: 0.4 }}
                      className={`w-full p-3 rounded-xl border bg-transparent text-sm focus:ring-2 focus:ring-blue-500/50 outline-none transition-all ${
                        (email && !isValidEmail(email)) || highlightEmail ? 'border-red-500/50 ring-2 ring-red-500/20' : ''
                      }`}
                      style={{ 
                        borderColor: (email && !isValidEmail(email)) || highlightEmail ? undefined : 'var(--card-border)', 
            ...
            <motion.div 
              ...
              style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--card-border)' }}
            >
              <SectionHeader icon={HelpCircle} title="FAQ" subtitle="Common Questions" />
              <div className="space-y-0">
            ...
            >
              ...
              <div className="flex flex-col items-center">
                <div className="bg-white rounded-2xl p-4 mb-4 shadow-md border border-black/5">
                  <QRCode url={installPageUrl} size={160} />
                </div>
                <p className="opacity-40 text-xs mb-4 font-medium uppercase tracking-wider">Scan QR Code</p>
                
                <div className="w-full flex items-center gap-2 rounded-xl p-2 border" style={{ backgroundColor: 'var(--input-bg)', borderColor: 'var(--card-border)' }}>
                  <code className="text-sm font-mono flex-1 truncate pl-2" style={{ color: 'var(--primary-color)' }}>{shortUrl}</code>
                  <button 
                    onClick={handleCopyLink}
                    className="p-2 hover:bg-white/10 rounded-lg transition-colors"
                  >
                    {linkCopied ? <Check className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4 opacity-40" />}
                  </button>
                </div>
              </div>
            </motion.div>
            ...
            {isMobile && platform === 'ios' && (
              ...
              >
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div className="flex flex-col">
                    <h2 className="text-xl font-bold tracking-tight" style={{ color: 'var(--primary-color)' }}>Installation Help</h2>
                    <span className="text-xs font-medium opacity-50 uppercase tracking-wider">Required Steps</span>
                  </div>
                </div>

                <div className="space-y-4">
                   <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/20 text-xs text-amber-500 leading-relaxed">
                     <span className="font-bold">Note:</span> iOS requires you to manually trust enterprise apps before they can run.
                   </div>
                   <ol className="space-y-4 text-sm font-medium relative">
                      ...
                      <li className="flex gap-4 items-start">
                        <span className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-black flex-shrink-0 ring-4 ring-[var(--card-bg)]" style={{ backgroundColor: 'var(--primary-color)', color: 'white' }}>1</span>
                        <div className="flex-1 pt-0.5">
                          <span className="opacity-70 block mb-1">Open Device Settings</span>
                          <span className="text-xs opacity-50 block">Go to <strong className="opacity-100">Settings → General</strong></span>
                        </div>
                      </li>
                      <li className="flex gap-4 items-start">
                        <span className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-black flex-shrink-0 ring-4 ring-[var(--card-bg)]" style={{ backgroundColor: 'var(--primary-color)', color: 'white' }}>2</span>
                         <div className="flex-1 pt-0.5">
                          <span className="opacity-70 block mb-1">Find Profile</span>
                          <span className="text-xs opacity-50 block">Tap <strong className="opacity-100">VPN & Device Management</strong></span>
                        </div>
                      </li>
                      <li className="flex gap-4 items-start">
                        <span className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-black flex-shrink-0 ring-4 ring-[var(--card-bg)]" style={{ backgroundColor: 'var(--primary-color)', color: 'white' }}>3</span>
                        <div className="flex-1 pt-0.5">
                          <span className="opacity-70 block mb-1">Trust Developer</span>
                          <span className="text-xs opacity-50 block">Select the profile and tap <strong className="opacity-100">Trust</strong></span>
                        </div>
                      </li>
                    </ol>
                </div>
        ...
        >
          {!data.hidePlatformBranding && (
            ...
            >
              Powered by <span className="font-bold tracking-tight" style={{ color: 'var(--primary-color)' }}>BetaDrop</span>
            </a>
          )}
        </motion.div>
      </div>
