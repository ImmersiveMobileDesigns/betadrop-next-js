...
const featureCategories: FeatureCategory[] = [
  {
    ...
    features: [
      ...
      },
      {
        ...
        description: 'Track every version with automatic latest/deprecated status. See your entire release timeline.',
      ...
      },
      {
        ...
        description: 'Pushed a bad build? Disable it and re-enable the previous version in one click.',
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description: 'Generate separate links for QA, stakeholders, and beta testers. Track who installs what.',
      ...
      },
      {
        ...
        description: 'Special links with built-in feedback forms. Collect bug reports and suggestions directly.',
    ...
    features: [
      {
        ...
        description: 'Downloads, unique devices, install success/failure rates, and geographic distribution per build.',
      ...
      },
      {
        ...
        description: 'See which iOS/Android versions, device models, and screen sizes your testers use.',
      ...
      },
      {
        ...
        description: 'Uploaded as guest before? Sign in and claim those builds to get full analytics retroactively.',
    ...
    features: [
      {
        ...
        description: 'Upload your logo, choose accent colors, and add custom messaging. Make install pages feel like yours.',
      ...
      },
      {
        ...
        description: 'All builds are unlisted. Only people with the link can access. Enable/disable builds anytime.',
...
export default function DeveloperFeatures() {
  return (
    <div id="features" className="mt-24 mb-12 scroll-mt-24 max-w-6xl mx-auto">
      <AnimateOnScroll animation="fadeUp" duration={700} threshold={0.2} className="text-center mb-16">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-blue-500/10 to-purple-500/10 border border-blue-500/20 mb-6">
          ...
          <span className="text-sm font-medium text-blue-300">Power User Features</span>
        </div>
        ...
        <p className="text-xl text-white/70 max-w-3xl mx-auto leading-relaxed">
          Guest uploads are perfect for quick sharing. But serious developers need serious tools.
          <br />
          <span className="text-white font-medium">Sign in once, unlock everything instantly.</span> No setup, no onboarding, just power.
        </p>
      </AnimateOnScroll>
        ...
        {featureCategories.map((category, categoryIndex) => (
          ...
          >
            <div>
              ...
              <div className={`grid ${category.features.length === 2 ? 'md:grid-cols-2' : 'md:grid-cols-3'} gap-6`}>
                {category.features.map((feature, featureIndex) => (
                  ...
                  >
                    ...
                    <div className={`mx-auto w-12 h-12 rounded-xl bg-gradient-to-br ${feature.iconGradient} flex items-center justify-center mb-4 group-hover:scale-110 transition-transform shadow-lg`}>
                    ...
                    {/* Benefit badge */}
                    <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full ${category.badgeColor} border ${category.badgeBorder}`}>
                      <Zap className={`w-3 h-3 ${category.iconColor}`} />
                      <span className={`text-xs font-medium ${category.badgeText}`}>{feature.benefit}</span>
                    </div>
                  </AnimateOnScroll>
                ))}
              </div>
      ...
      {/* Comparison Callout */}
      <AnimateOnScroll animation="scaleIn" duration={700} delay={100} threshold={0.2} className="mt-20">
        <div className="p-8 rounded-2xl bg-gradient-to-br from-blue-500/10 via-purple-500/10 to-pink-500/10 border border-white/10 backdrop-blur-sm">
          <div className="max-w-4xl mx-auto">
            <h3 className="text-2xl font-bold text-white mb-6 text-center">Why Developers Choose BetaDrop</h3>
            <div className="grid md:grid-cols-3 gap-8 text-center">
              {[
                { name: 'TestFlight', desc: '90-day review wait, 100 tester limit, iOS only' },
                { name: 'Firebase App Distribution', desc: 'Complex setup, requires Google account, limited analytics' },
                { name: 'Diawi', desc: '7-day expiry, no build management, basic features only' },
              ].map((competitor, i) => (
                <AnimateOnScroll key={i} animation="fadeUp" duration={500} delay={i * 100} threshold={0.2}>
                  <div className="text-white/40 text-sm font-medium mb-2">{competitor.name}</div>
                  <div className="text-white/60 text-sm">{competitor.desc}</div>
                </AnimateOnScroll>
              ))}
            </div>
            <div className="mt-8 pt-8 border-t border-white/10 text-center">
              <AnimateOnScroll animation="fadeUp" duration={600} delay={200} threshold={0.2}>
                <div className="text-xl font-bold text-white mb-2">BetaDrop</div>
                <div className="text-white/80">
                  Instant setup, unlimited builds, advanced analytics, full control.{' '}
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-500 font-semibold">All free, forever.</span>
                </div>
              </AnimateOnScroll>
            </div>
          </div>
      ...
      {/* Enhanced CTA */}
      <AnimateOnScroll animation="scaleIn" duration={700} delay={100} threshold={0.2} className="text-center mt-16">
        <Link 
          href="/login" 
        ...
        <p className="text-white/50 text-sm mt-4">
          No credit card. No setup wizard. Just sign in with Google and start shipping.
        </p>
      </AnimateOnScroll>
    </div>
  );
